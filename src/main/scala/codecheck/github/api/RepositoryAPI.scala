...
case class RepositoryAPI(api: GitHubAPI, owner: String, repo: String) {
  ...
  //LabelOp
  def addLabels(number: Long, labels: String*): Future[Iterable[Label]] =
    api.addLabels(owner, repo, number, labels:_*)

  def replaceLabels(number: Long, labels: String*): Future[Iterable[Label]] =
    api.replaceLabels(owner, repo, number, labels:_*)

  def removeAllLabels(number: Long): Future[Iterable[Label]] =
    api.removeAllLabels(owner, repo, number)

  def removeLabel(number: Long, label: String): Future[Iterable[Label]] =
    api.removeLabel(owner, repo, number, label)

  def listLabels(number: Long): Future[Iterable[Label]] =
    api.listLabels(owner, repo, number)

  def listLabelDefs: Future[Iterable[Label]] =
  ...
  //MilestoneOp
  def listMilestones(option: MilestoneListOption = MilestoneListOption()): Future[Iterable[Milestone]] =
  ...
  // PullRequestOp
  def listPullRequests(option: PullRequestListOption = PullRequestListOption()): Future[Iterable[PullRequest]] =
