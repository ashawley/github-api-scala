...
trait RepositoryOp {
  ...
  private def doList(path: String, option: RepositoryListOption): Future[Iterable[Repository]] = {
  ...
  def listOwnRepositories(option: RepositoryListOption = RepositoryListOption()): Future[Iterable[Repository]] =
    doList("/user/repos", option)

  def listUserRepositories(user: String, option: RepositoryListOption = RepositoryListOption()): Future[Iterable[Repository]] =
    doList(s"/users/$user/repos", option)

  def listOrgRepositories(org: String, option: RepositoryListOption = RepositoryListOption()): Future[Iterable[Repository]] =
    doList(s"/orgs/$org/repos", option)

  def listAllPublicRepositories(since: Long = 0): Future[Iterable[Repository]] = {
