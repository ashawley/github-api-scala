...
trait IssueOp {
  ...
  private def doList(path: String): Future[Iterable[Issue]] = {
    exec("GET", path).map(
      _.body match {
  ...
  def listAllIssues(option: IssueListOption = IssueListOption()): Future[Iterable[Issue]] =
    doList("/issues" + option.q)

  def listUserIssues(option: IssueListOption = IssueListOption()): Future[Iterable[Issue]] =
    doList("/user/issues" + option.q)

  def listOrgIssues(org: String, option: IssueListOption = IssueListOption()): Future[Iterable[Issue]] =
    doList(s"/orgs/$org/issues" + option.q)

  def listRepositoryIssues(owner: String, repo: String, option: IssueListOption4Repository = IssueListOption4Repository()): Future[Iterable[Issue]] =
